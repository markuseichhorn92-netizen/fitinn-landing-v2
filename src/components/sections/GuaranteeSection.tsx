...
export function GuaranteeSection() {
  ...
  return (
    <section className="py-7 relative overflow-hidden" ref={(node) => { section.ref(node); float.ref(node) }}>
      ...
      <FloatingDecor position={{ top: '12%', right: '5%' }} isVisible={float.isVisible} progress={float.progress} delay={0.2} parallax={-50} sizeClass="w-5 h-5 md:w-11 md:h-11 lg:w-14 lg:h-14">
        <ShieldCheckSvg className="w-full h-full text-primary" />
      </FloatingDecor>
      <FloatingDecor position={{ bottom: '15%', left: '4%' }} isVisible={float.isVisible} progress={float.progress} delay={0.4} parallax={60} sizeClass="w-4 h-4 md:w-10 md:h-10 lg:w-12 lg:h-12">
        <ShieldCheckSvg className="w-full h-full text-primary" />
      </FloatingDecor>
